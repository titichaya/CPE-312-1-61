...
void LCD_GLASS_Init(void)
{
  ...
  LCD_Init(&LCD_InitStruct);												//Initialize the LCD
  LCD_MuxSegmentCmd(ENABLE);
  LCD_ContrastConfig(LCD_Contrast_Level_4);					//To set contrast to mean value
  LCD_DeadTimeConfig(LCD_DeadTime_0);
  LCD_PulseOnDurationConfig(LCD_PulseOnDuration_4);
  LCD_WaitForSynchro();			//Wait Until the LCD FCR register is synchronized
  LCD_Cmd(ENABLE);					//Enable LCD peripheral
  while(LCD_GetFlagStatus(LCD_FLAG_ENS) == RESET);	//Wait Until the LCD is enabled
  while(LCD_GetFlagStatus(LCD_FLAG_RDY) == RESET);	//Wait Until the LCD Booster is ready
...
{  
  ...
  LCD->RAM[LCD_RAMRegister_4] |= (uint32_t)(t_bar[0]<<12);	//bar1 bar3
  LCD->RAM[LCD_RAMRegister_6] |= (uint32_t)(t_bar[1]<<12);	//bar0 bar2
}
...
void LCD_GLASS_WriteChar(uint8_t* ch, bool point, bool column, uint8_t position)
{
  ...
  LCD_Conv_Char_Seg(ch,point,column,digit);	//To convert displayed character in segment in array digit
  switch (position)
  {
...
void LCD_GLASS_DisplayString(uint8_t* ptr)
{
  ...
  while( LCD_GetFlagStatus (LCD_FLAG_UDR) != RESET);	//wait for LCD Ready
  while ((*ptr != 0) & (i < 8))	//Send the string character by character on lCD
  {
    LCD_GLASS_WriteChar(ptr, FALSE, FALSE, i);	//Display one character on LCD
    ptr++;			//Point on the next character
    i++;				//Increment the character counter
  }
  LCD_UpdateDisplayRequest();		//Update the LCD display
}
...
void LCD_GLASS_ScrollSentence(uint8_t* ptr, uint16_t nScroll, uint16_t ScrollSpeed)
{
  ...
  uint8_t Str_size;
  if (ptr == 0) return;
  for (ptr1=ptr,Str_size = 0 ; *ptr1 != 0; Str_size++,ptr1++) ;	//To calculate end of string
  ptr1 = ptr;
  LCD_GLASS_DisplayString(ptr);
  Delay(ScrollSpeed);
  for (Repetition=0; Repetition<nScroll; Repetition++){	//To shift the string for scrolling display
    for (Char_Nb=0; Char_Nb<Str_size; Char_Nb++){
      ...
      *(str+5) =* (ptr1+((Char_Nb+6)%Str_size));
      ...
      if (KeyPressed)	//user button pressed stop the scrolling sentence
				return;   		
      Delay(ScrollSpeed);
    }	
  }
